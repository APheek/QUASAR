...
public class HikOffset : MonoBehaviour
{
    ...
    IEnumerator Waiting() //Como todos los codigos tendrian lo mismo, hacemos una funcion para todas. 
    {
        ...
        {
            float ceta = Mathf.Lerp(hikari.value2, inicial2, 1f * Time.deltaTime);
            hikari.value2 = ceta;

            float smoothedPosition = Mathf.Lerp(hikari.value, inicial, 1f * Time.deltaTime);
            hikari.value = smoothedPosition;
            yield return null;
